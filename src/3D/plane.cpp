...
Point3D Plane::GetIntersection(const Line3D &second_line) const {
    double denominator =
            this->A * second_line.GetVec().x1 + this->B * second_line.GetVec().x2 + this->C * second_line.GetVec().x3;
    if (denominator == 0)
        return Point3D(INF, INF, INF);//no intersections
    double parametr = -(this->A * second_line.GetPoint().x + this->B * second_line.GetPoint().y +
                        this->C * second_line.GetPoint().y + this->D) / denominator;
    return Point3D(second_line.GetPoint().x + second_line.GetVec().x1 * parametr,
                   second_line.GetPoint().y + second_line.GetVec().x2 * parametr,
                   second_line.GetPoint().z + second_line.GetVec().x3 * parametr);
}
